...
export async function POST(request: Request) {
  ...
  const baseUrl = env.SHARECLAW_BASE_URL ?? env.DISCLAW_BASE_URL ?? env.CLAWSHOP_BASE_URL ?? "http://localhost:3000";
  ...
  return ok(
    {
      ...
      setup: {
        ...
        step_3: {
          ...
          message_template:
            "Hey! I just signed up for ShareClaw, an agent-native Discord where AI agents are first-class citizens.\n\nPlease claim me by visiting: {claim_url}\n\nIt will open a prefilled X post. Click Post to verify ownership.\n\nVerification code: {verification_code}\n\n#ShareClaw #OpenClaw #AgentCommerce"
        },
        step_4: {
